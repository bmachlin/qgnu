...
let seekTime = defaultSeekTime;                 // time to seek forward or back when pressing seek keys
let skipBackBuffer = defaultSkipBackBuffer;     // buffer time to after a marker's time has passed 
...
function loadSettings() {
    ...
    let sh = storage.getItem("show-heading")
    if (sh == null || sh === "true") {
...
function saveSettings(st=null, sbb=null, thm=null) {
    ...
    storage.setItem('theme', thm ?? document.getElementById("themeLight").checked ? "light" : "dark");
    console.log("saved settings", (st ?? seekTime), (sbb ?? skipBackBuffer), (thm ?? theme));
    loadSettings();
}

function defaultSettings() {
    saveSettings(defaultSeekTime, defaultSkipBackBuffer, defaultTheme);
}
...
function toggleHeading() {
    setHeadingVisible(document.getElementById("heading").hidden);
}
...
function setHeadingVisible(visible) {
    ...
    storage.setItem("show-heading", visible);
}
...
function displayTab(tab) {
    let lines = tab.split('\n');
    let display = document.getElementById('tab-text');
    let markerBar = document.getElementById("marker-bar");
    display.innerHTML = '';
    for (let li = 0; li < lines.length; li++) {
        let matches = lines[li].match(markerRegex);
        if (matches != null && matches.length == 1) {
            // add marker element
            let marker = markerStringToObject(matches[0]);
            let el = MarkerElement(marker, null);
            markerBar.appendChild(el);
            lines[li] = lines[li].replace(markerRegex, "")
        } else {
            // add empty element
            markerBar.appendChild(document.createElement('br'));
        }
